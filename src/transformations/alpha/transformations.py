...
alpha_transformations = A.Compose([
  A.Resize(512, 512),  # This transform resizes each image before subsequent processing
  A.HorizontalFlip(p=0.5),  # This transform flips each image horizontally with a probability of 0.5
  A.VerticalFlip(p=0.5),  # This transform flips each image horizontally with a probability of 0.5
